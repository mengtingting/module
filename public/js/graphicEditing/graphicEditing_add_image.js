...
function initializationOfGraphicEditing(data){
    ...
    if(config_graphic_using['number']>0){
        ...
        if(config_graphic_using['number']==1){
            ...
            else if(config_graphic_using['images']){
                module='images'
            }
...
function LoadDetailsHtml(data){
    ...
    for(var i=0;i<data.length;i++){
        ...
        if(data[i]['type']==0){
            data_text[i]['image']=reverseRow(data[i]['image']);
            //编辑
            ...
            data_text[i]['image']=transformationRow(data_text[i]['image']);
            //展示
...
function updateTextDetial(index){
    var image=$('#'+config_graphic_using['prefix']+'text_detail_'+index).val();
    image=transformationRow(image)
    jsonObj[index]['image']=image;
    for(var i=0;i<jsonObj.length;i++){
        editExampleDetailList(jsonObj[i])
    }
    //重新展示
